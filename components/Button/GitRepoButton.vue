...
if (props.repositories) {
  repositories = props.repositories.map((repository: GitRepository) => ({
...
<template>
  <template v-if="repositories.length > 1">
    ...
    <Menu
      ...
      :model="items"
      :popup="true"
      class="bg-gray-800 border border-slate-500 border-1 rounded-md mt-[1px] ml-1"
      :unstyled="true"
    >
      <template #item="{ item }">
        <NuxtLink
          :to="item.url"
          target="_blank"
          class="block px-2 py-3 rounded hover:bg-slate-700 hover:bg-opacity-50"
        >
