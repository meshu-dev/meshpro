...
<template>
  ...
  <div class="flex flex-col mt-16 gap-12 text-2xl mb-12">
    <div
      v-for="blogPost in blogPostData"
      class="flex justify-between max-lg:flex-col max-lg:justify-start"
    >
      <a
        :href="`${blogUrl}/${blogPost.slug}`"
        target="_blank"
        class="text-sky-400 decoration-dotted underline underline-offset-8 leading-10"
        >{{ blogPost.title }}</a
      >
