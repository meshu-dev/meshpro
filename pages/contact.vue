...
const sendContactMsg = async () => {
  if (hasFields(contactMsg)) {
    const response: ContactMessageResponse | null = await sendMessage(
      toRaw(contactMsg.value),
    );
...
const onSubmit = async () => {
  ...
  if (token) {
    ...
    statusMsg.value = {
      type: "error",
      text: "An error occurred validating submission. Please try again later.",
    } as StatusMsg;
  }
  return;
};
...
<template>
  <div class="flex gap-20 max-lg:flex-col">
    <div>
      ...
      <div class="text-2xl">
        <p class="leading-9">
          I'm always on the lookout for new opportunities so please send me a
    ...
    <form @submit.prevent="onSubmit">
      <Message v-if="statusMsg" :severity="statusMsg.type">{{
        statusMsg.text
      }}</Message>
      <div class="mb-5">
      ...
      <div class="mb-5">
        ...
        <Textarea
          class="w-[900px] h-[400px] resize-none max-xl:w-[700px] max-lg:w-full"
          rows="5"
          cols="30"
          placeholder="Message"
          v-model="contactMsg.message"
        />
      </div>
