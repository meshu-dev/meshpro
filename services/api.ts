...
export const getIntroText = async (token: string = ""): Promise<Intro> => {
  ...
  const { data } = await useAsyncData("intro", () =>
    $fetch(`${apiUrl}/portfolio/intro`, {
      method: "GET",
    }),
  );
...
export const getAboutData = async (token: string = ""): Promise<About> => {
  const apiUrl: string = getPortfolioApiUrl();
  const { data, pending, error, refresh } = await useAsyncData("about", () =>
    $fetch(`${apiUrl}/portfolio/about`, {
      method: "GET",
    }),
  );
  const apiData: ApiResponse = toRaw(data.value) as ApiResponse;
  ...
  await useFetch(`${apiUrl}/contact`, {
    ...
    onResponse({ request, response, options }) {
      ...
      if (response.status == 200) {
        const responseData: { errors: any; message: string } = toRaw(
          response._data,
        );
